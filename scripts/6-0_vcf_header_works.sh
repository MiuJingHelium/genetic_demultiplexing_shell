...
for sample in ${SAMPLES[@]}; do
    ...
    header="$bulk_dir/$sample/header.vcf"
    head -n 257 ${input} > ${header}
     ...
     LSF_DOCKER_PRESERVE_ENVIRONMENT=false bsub -q martyomov -G compute-martyomov \
            ...
            "/usr/bin/picard-tools FixVcfHeader I=${header} O=${header}"

    vcf="$bulk_dir/$sample/only_unique_snp.vcf" 
    out_vcf="$bulk_dir/$sample/only_unique_snp_header.vcf"
    cat ${header} ${vcf} > ${out_vcf} # store out vcf to bulk
